...
test("keeps multiplayer, 3D tiles, continuous wall, and fan scoring wired", async () => {
  ...
  assert.match(client, /function TimedFanReveal/);
  assert.match(client, /function TurnStatus/);
  assert.doesNotMatch(client, /setInterval\(\(\) => setClock/);
  assert.match(client, /function WinningHandReview/);
  assert.match(client, /胡牌确认 · 按先后顺序/);
  assert.match(client, /查看本局结算/);
  assert.match(client, /snapshot\.result\.finishedAt/);
  assert.match(server, /concealedTiles: sortTiles\(\[\.\.\.winner\.hand\]\)/);
  assert.match(server, /melds: winner\.melds\.map/);
  assert.match(server, /room\.result = \{ reason, finishedAt, standings, settlements: room\.settlements \}/);
  assert.match(server, /lastAcquiredTile/);
  ...
  assert.match(tableStyles, /\.game-shell > \.result-layer \.result-panel/);
  assert.match(tableStyles, /\.winning-hand-review/);
  assert.match(tableStyles, /\.result-reopen/);
